...
/**
 * Abstract base class for Eclipse Workbench Actions which run based on the Palladio workflow
 * engine, i.e., the run has an IJob which gets executed. The class offers features to support
 * logging into the Eclipse console, support for an Eclipse process which can be used to interrupt,
 * terminate or debug the run, integration of the Eclipse progress bar, exception handling, etc. *
...
public abstract class AbstractWorkbenchDelegate<WorkflowConfigurationType extends AbstractJobConfiguration, WorkflowType extends Workflow>
    ...
    protected final List<WorkflowStatusListener> workflowListener = new ArrayList<>();

    /**
     * The progress monitor to set in the workflow and to report any process progress to.
     */
    ...
    protected WorkflowType createWorkflow(WorkflowConfigurationType workflowConfiguration) {
        return (WorkflowType) new Workflow(createWorkflowJob(workflowConfiguration), progressMonitor,
    ...
    protected LoggerAppenderStruct setupLogger(String loggerName, Level logLevel, String layout) {
        ...
        return new LoggerAppenderStruct(loggerInstance, logLevel, appender);
    }
    ...
    /**
     * Set another progress monitor the workflow should report to than the default null progress
     * one.
     *
     ...
     * to return a strongly typed configuration object needed by this workflow's main workflow job.
     *
