 ...
 * an IJob which gets executed. The class offers features to convert the
 * information contained in ILaunchConfiguration into a strongly typed
 * configuration object for the workflow job, features to suport logging into
 * the Eclipse console, support for an Eclipse process which can be used to
 * interrupt, terminate or debug the run, integration of the Eclipse progress
 * bar, execption handling, etc.
 * 
 * The class is supposed to be used to implement Eclipse run configurations,
 * like SimuCom, ProtoCom, PCM2Java, PCM2LQN, SoMoX, Java2PCM, etc.
 * 
 * The class is abstract and defines some methods, which must be implemented by
...
public abstract class AbstractWorkflowBasedLaunchConfigurationDelegate<WorkflowConfigurationType extends AbstractWorkflowBasedRunConfiguration, WorkflowType extends Workflow>
	...
	protected Level getLogLevel(ILaunchConfiguration configuration) {
		try {
			switch (configuration.getAttribute(
					DebugEnabledCommonTab.WORKFLOW_ENGINE_DEBUG_LEVEL, 0)) {
			case 0:
				return Level.TRACE;
			case 1:
				return Level.DEBUG;
	 ...
	 *            main workflow job
	 * @param launch
	 *            The eclipse ILaunch associated to the workflow, needed to
	 *            setup debugging if the workflow supports it
	 ...
	 * clients. Its purpose is to convert the Eclipse ILaunchConfiguration
	 * (which is basically an untyped hashmap) into a strongly typed
	 * configuration object needed by this workflow's main workflow job.
	 * 
	 * @param configuration
	 *            The ILaunchConfiguration to be converted into a strongly typed
	 *            configuration object for the main workflow job
	 * @param mode
	 ...
	 *             the core exception
	 */
	protected abstract WorkflowConfigurationType deriveConfiguration(
