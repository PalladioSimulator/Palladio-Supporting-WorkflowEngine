...
/**
 * A abstract QVTR engine implementing {@link QVTREngine}. This implements convenience operations
 * for a QVT-R engine. QVTREngine implementations can derive this class. It is not intended to
...
public abstract class AbstractQVTREngine implements QVTREngine {
    ...
    /**
     * Derived engines should implement this to provide the information for a give QVT-R script
     * file.
     *
    ...
    /**
     * Creates a new QVTRScriptInfoImpl. Derived engines can use this to create the
     * QVTRScriptInfoImpl.
     *
     * @param transformationInfo
     *            A {@link HashMap} of transformations and their directions
     * @param valid
     *            is the script file valid
    ...
    /**
     * Returns a registered QVT-R engine for a given id. If the engine with the id is not available
     * null is returned.
     *
     * @param id
     *            a id of a QVT-R engine
     * @return an implementation of the {@link QVTREngine}
     */
    public static AbstractQVTREngine getInstance(final String id) {
        final QVTREngines[] engines = QVTREngines.getAllEngines();
        // search for given id
        for (final QVTREngines engine : engines) {
            // if id found or no id provided return new engine
            if ((id == null) || (engine.getId() == id)) {
                return engine.newQVTREngine();
            }
        }
    ...
    public static Collection<String> getAllInstanceIDs() {
        final ArrayList<String> engineIDs = new ArrayList<String>();
        final QVTREngines[] engines = QVTREngines.getAllEngines();
        // enumerate all ids
        for (final QVTREngines engine : engines) {
    ...
    protected static final class QVTRScriptInfoImpl {
         ...
         *
         * @param transformationInfo
         *            the map of transformations and directions
         * @param valid
         *            information if the script is valid
         */
         ...
         *            the transformation name to query
         * @return the direction for a given transformation
         */
        String[] getDirections(final String transformationName) {
            final ArrayList<String> directions = this.transformationInfo.get(transformationName);
            if (directions != null) {
