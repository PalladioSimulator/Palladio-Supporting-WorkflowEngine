...
/**
 * Abstract base class for Eclipse Workbench Actions which run based on the SDQ workflow engine,
 * i.e., the run has an IJob which gets executed. The class offers features to support logging into
 * the Eclipse console, support for an Eclipse process which can be used to interrupt, terminate or
 * debug the run, integration of the Eclipse progress bar, execption handling, etc. * The class is
 * abstract and defines some methods, which must be implemented by subclasses. See method
 ...
 */
public abstract class AbstractWorkbenchDelegate<WorkflowConfigurationType 
        extends AbstractJobConfiguration, WorkflowType extends Workflow>
    ...
    /**
     * Get the log level based on the extended CommonTab in DebugEnabledCommonTab TODO: Anne has set
     * this to protected because the logging has to be re-enabled during PerOpteryx. Check later
