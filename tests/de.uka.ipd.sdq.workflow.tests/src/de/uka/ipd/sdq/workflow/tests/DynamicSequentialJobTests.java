 ...
 */
public class DynamicSequentialJobTests {
    ...
    @Before
    public void setUp() {
        myCompJob = new DynamicSequentialJob();
    ...
    /**
     * add a number of jobs. execute the composite job. After half of the jobs are executed, add some more. 
     ...
     */
    @Test
    public void testInOrderExecutionWithDynamicExtension() throws JobFailedException, UserCanceledException {
        final LinkedList<MockJob> jobs = new LinkedList<MockJob>();
        final NullProgressMonitor monitor = new NullProgressMonitor();
    ...
    @Test
    public void testJobAddedDuringCleanupIsNotExecuted() throws JobFailedException, UserCanceledException, CleanupFailedException {
        final MockJob job = new MockJob();
        myCompJob = new DynamicSequentialJob(false);
        myCompJob.addJob(new IJob() {            
            @Override
            public String getName() {
                return "";
            }
        ...
        myCompJob.execute(monitor);
        assertEquals(false, job.wasExecuted());

        myCompJob.cleanup(monitor);
        assertEquals(false, job.wasCleanedUp());
    }
